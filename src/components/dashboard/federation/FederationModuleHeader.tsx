...
import { Globe, Network, Cloud, Shield } from "lucide-react";

interface FederationModuleHeaderProps {
  systemName: string;
  moduleId: string;
  moduleName: string;
  version: string;
...
export function FederationModuleHeader({
  systemName,
  moduleId,
  moduleName,
  version,
  status,
  description,
}: FederationModuleHeaderProps) {
  return (
    <div className="relative overflow-hidden">
      {/* Background glow effect */}
      <div className="absolute inset-0 bg-gradient-to-br from-accent/10 via-transparent to-warning/10 pointer-events-none" />
      ...
      <div className="relative z-10 py-12 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-10 animate-slide-up animation-delay-600">
            {[
              { icon: Globe, label: "Active Regions", value: "4" },
              { icon: Network, label: "Federated Orgs", value: "23" },
              { icon: Cloud, label: "Cloud Providers", value: "5" },
